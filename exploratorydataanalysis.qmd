...
ggplot(data = top_partners, aes(x = reorder(partner_name, -trade_difference), y = trade_difference, fill = factor(sign(trade_difference)))) +
  ...
  scale_fill_manual(values = c("red", "blue"), name = "",
                    labels = c("Turkey exported more", "Turkey imported more")) +
  labs(title = "Trade Comparison with Top 10 Trading Partners(2015-2020)",
       x = "Partner",
       y = "Trade Difference (Export - Import, USD)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        legend.position = "top")
```
...
filtered_commodity_data$truncated_name <- str_trunc(filtered_commodity_data$section_name, 120)

# Create a pie chart
ggplot(filtered_commodity_data, aes(x = "", y = total_trade_value_5_year, fill = truncated_name)) +
